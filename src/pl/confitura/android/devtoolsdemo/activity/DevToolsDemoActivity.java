...
public class DevToolsDemoActivity extends RoboActivity {
	...
	// Resources
	@InjectResource(R.anim.spin)
	Animation spin;
	@InjectResource(R.integer.max_punches)
	int MAX_PUNCHES;

	// Other Injections
	@Inject
	ChangeTextAnimationListener changeTextAnimationListener;
 ...
 * annotations will be processed, eg. helloView and fist will both be injected.
 * 
 * Since we haven't customized it, first will be injected with an instance
 * created using the default constructor for Fist.
 */
class ChangeTextAnimationListener implements AnimationListener {
	...
	public void onAnimationEnd(Animation animation) {
		helloView.setText(info.versionName); // bonus surprise from
												// AndroidManifest
	}
}
 ...
 * 
 * Again, "random" and "context" will be injected at the time this is injected
 * into ChangeTextAnimationListener.
 * 
 * And again, the default constructor will be used for random.
 * 
 * However, RoboGuice ships with a custom binding for Context, so context will
 * be injected with the current context (aka the Batgirl activity instance in
 * this case)
 */
class Fist {
	@Inject
